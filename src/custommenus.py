...
class CustomMenus:
    ...
    def c_addEntry(self):
        ...
        if self.correspEntry[index][0] != 1: return
    ...
    def c_addSubmenu(self):
        ...
        if self.correspEntry[index][0] == 0:
            ...
            insertIndex = index + self.count(self.correspEntry[index][1])  \
                          - len(self.correspEntry[index][1]['entries']) - 1
    ...
    def c_delete(self):
        ...
        if not self.current: return
        index = int(self.current[0])
        if self.correspEntry[index][0] == 0: return
    ...
    def c_addPattern(self):
        ...
        if self.master.board.selection[0][0] > self.master.board.selection[1][0] or \
           self.master.board.selection[0][1] > self.master.board.selection[1][1]:
            self.master.board.selection = ((1,1), (19, 19))
        ...
        for i in range(sel[0][1], sel[1][1]+1):
            for j in range(sel[0][0], sel[1][0]+1):
                if self.master.board.getStatus(j,i) == 'W': d[(i-1,j-1)] = 'O'
                elif self.master.board.getStatus(j,i) == 'B': d[(i-1,j-1)] = 'X'
                else: d[(i-1,j-1)] = '.'
                if (j,i) in self.master.board.wildcards: d[(i-1,j-1)] = '*' # FIXME distinguish different wildcards
    ...
    def pollList(self):
        ...
        if not now and self.current:
            ...
            for widget in [self.addEntryB, self.addSubmenuB, self.deleteB,
                           ...
                           self.patternB, self.giB, self.resetB]: widget.config(state=DISABLED)
