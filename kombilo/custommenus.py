    ...
    def buildMenus(self, reload=1, alternativePath=''):
        ...
        if reload:
            ...
            try:
                ...
                if alternativePath:
                    try:
                        file = open(os.path.join(alternativePath, 'menus.def'), 'rb')
                        self.customMenuList = pickle.load(file)
                        file.close()
                    except IOError:
                ...
                try:
                    if ('AB' in self.master.cursor.currentNode() or 'AW' in self.master.cursor.currentNode()
                       or 'AE' in self.master.cursor.currentNode()):
                        ...
                        self.master.capVar.set(_('Cap - B: %d, W: %d') % (self.master.capB, self.master.capW))
                        ...
                        self.master.moveno.set('%d' % (int(self.master.moveno.get()) + 1))
